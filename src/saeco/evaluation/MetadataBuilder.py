...
#         self.unique_labels = {}  # for strings only
#         self.filter = filter
#         assert shape[0] == filter.filter.shape[0]
#         vshape = (filter.filter.mask.sum(),) + shape[1:]
#         self._value = FilteredTensor.from_value_and_mask(
#             torch.zeros(*vshape, dtype=dtype, device=device), filter
#         )
