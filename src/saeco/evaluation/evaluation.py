...
@define(slots=True)
class Evaluation[InputsT: torch.Tensor | DictBatch](Enrichment, Patching, Coactivity):
    model_path: Path
    ...
    def _tokenizer_default(self) -> PreTrainedTokenizerFast:
        return self.run_cfg.train_cfg.data_cfg.model_cfg.tokenizer

    tokenizer: PreTrainedTokenizerFast = field(default=Factory(_tokenizer_default, takes_self=True))

    def _model_adapter_default(self):
        model_kwargs = getattr(self.run_cfg.train_cfg.data_cfg.model_cfg, "model_kwargs", {})
        try:
            from saeco.data.config.model_config.comlm_model_cfg import ComlmModelConfig
            if isinstance(self.run_cfg.train_cfg.data_cfg.model_cfg.model_load_cfg, ComlmModelConfig):  # type: ignore[attr-defined]
                return ComlmEvalAdapter(model_kwargs=model_kwargs)
        except Exception:
            pass
        return LanguageModelEvalAdapter(model_kwargs=model_kwargs)

    model_adapter: ModelEvalAdapter = field(default=Factory(_model_adapter_default, takes_self=True))
    ...
    def open_cache(cls, name: Path | str) -> "Evaluation":
        ...
        inst = cls.open_model(cached.cfg.model_path, averaged_weights=cached.cfg.averaged_model_weights)
        inst.cached_acts = cached
        return inst

    @classmethod
    def open_model(cls, path: Path | str, *, averaged_weights: bool = False) -> "Evaluation":
        p = path if isinstance(path, Path) else Path(path)
        arch = Architecture.load(p, load_weights=True, averaged_weights=averaged_weights)
        return cls(architecture=arch, model_path=p, averaged_model_weights=averaged_weights)
    ...
    def path(self) -> Path:
        if self.cached_acts is None:
            raise ValueError("No cached_acts loaded; call Evaluation.open_cache(...) first")
    ...
    def metadatas(self) -> Metadatas:
        if self.filter is not None:
            raise ValueError("Use eval.metadata[...] on filtered evals (metadatas store is root-only)")
    ...
    def token_strings(self, tokens: int | list[int] | Tensor | DictBatch) -> str | list[str] | list[list[str]]:
        if isinstance(tokens, int):
    ...
    def top_activations(self, f: int | FilteredTensor, *, k: int | None = None, p: float | None = None, agg=None) -> TopActivations:
        feat = self.feature(f)
        if agg is None:
    ...
    def store_acts(self, cfg: CacheConfig, displace_existing: bool = False):
        ...
        cacher = ActsCacher.from_cache_and_runner(caching_config=cfg, architecture=self.architecture)

        if cacher.path.exists():
            if not displace_existing:
                raise FileExistsError(f"{cacher.path} already exists. Use displace_existing=True.")
            import time
