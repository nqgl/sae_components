...
class HSMix(cl.Module):
    def __init__(self, p_soft: float):
        super().__init__()
        self.p_soft = p_soft
        self.hard = ReuseForward(co.ops.Thresh())
...
class MultiGate(cl.Module):
    def __init__(self, n_sections=3, targets: float | list[float] = 5):
        ...
        self.gates = ReuseForward(
            cl.Router(
                [nn.ReLU()] + [ReuseForward(HSMix(0.5)) for _ in range(n_sections - 1)]
            ).reduce(lambda *l: l)
        )

        self.out = ReuseForward(
            cl.Parallel(self.gates).reduce(lambda a, b: a * b, binary=True)
        )
        self.penalizers = [None] + [L0TargetingL1Penalty(target=t) for t in targets]
        self.update_penalty_l0s = cl.Parallel(
            *[
                cl.Seq(
                    mask=cl.Seq(
                        co.ops.Lambda(lambda l: l[i:]),
                        cl.Router(
                            *[self.gates.module[j].hard for j in range(i, n_sections)]
                        ).reduce(lambda a, b: a * b, binary=True),
                    ),
                    L0=co.metrics.L0(),
                    update_penalty=self.penalizers[i],
                )
        ...
        self.apply_penalty = Seq(
            cl.Parallel(
                ...
                lambda out_det, gates: [
                    out_det * gates[i] / (gates[i].detact() + eps)
                    for i in range(1, n_sections)
                ],
            ),
            cl.Router(*[self.penalizers[i] for i in range(1, n_sections)]),
        )
    ...
    def forward(self, pre_acts_list: list[torch.Tensor], *, cache: cl.Cache, **kwargs):
        ...
        l0s = cache(self).update_penalty_l0s(pre_acts_list[1:])
